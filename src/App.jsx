...
function App() {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        const response = await fetch('http://100.99.97.126:5173/api/tasks');
        const data = await response.json();
        setTasks(decayActiveTasks(data));
      } catch (e) {
        console.error('Error loading tasks:', e);
        // Fallback to seed data if backend is unavailable
        setTasks(decayActiveTasks(seedTasks()));
      } finally {
        setLoading(false);
      }
    };
  ...
  const updateTask = async (id, fields) => {
    try {
      await fetch(`http://100.99.97.126:5173/api/tasks/${id}`, {
        method: 'PUT',
  ...
  const handleAction = async (id, type, extra = {}) => {
    try {
      const t = now();
      let updateData = {};
      ...
      switch (type) {
        case "relevant":
          updateData = {
            ...
            last_reviewed_at: t,
            relevance_score: (tasks.find(t => t.id === id)?.relevance_score || 0) + 5,
            updated_at: t,
          };
          break;
        case "notNow":
        case "snooze": {
          ...
          updateData = {
            ...
            last_reviewed_at: t,
            relevance_score: (tasks.find(t => t.id === id)?.relevance_score || 0) - 3,
            updated_at: t,
          };
          break;
        }
        case "done":
          updateData = { status: "archived", archived_at: t, updated_at: t };
          break;
        case "archive":
          updateData = { status: "archived", archived_at: t, updated_at: t };
          break;
        case "delete":
          await fetch(`http://100.99.97.126:5173/api/tasks/${id}`, { method: 'DELETE' });
          const refreshed = await fetch('http://100.99.97.126:5173/api/tasks');
          setTasks(decayActiveTasks(await refreshed.json()));
          return;
        case "restore":
          updateData = { status: "vault", archived_at: null, updated_at: t };
          break;
        case "moveToToday":
          updateData = { status: "today", updated_at: t };
          break;
        case "undoToday":
          updateData = { status: "active", updated_at: t };
          break;
        case "backToVault":
          updateData = { status: "vault", updated_at: t };
          break;
      }
